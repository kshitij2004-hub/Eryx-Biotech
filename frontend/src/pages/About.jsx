...
function About() {
  ...
  const fetchTeam = async () => {
    try {
      ...
      console.error("Failed to extract active corporate roster, loading system fallbacks:", err);
      
      // 👥 Fallback Roster: Ensures your UI stays populated if the server goes offline
      setTeam([
        {
          id: 'fb-1',
          name: 'Dr. Corporate Administrator',
          role: 'Chief Executive Officer',
          department: 'Executive Board',
          bio: 'Leading operational parameters and corporate strategic directions globally.',
          image_url: null
        },
        {
          id: 'fb-2',
          name: 'Technical Director Node',
          role: 'Head of R&D',
          department: 'Research & Development',
          bio: 'Synchronizing biological development processes with advanced formulations.',
          image_url: null
        }
      ]);
    } finally {
  ...
  const pageContainer = { padding: '60px max(5%, 20px)', maxWidth: '1200px', margin: '0 auto', fontFamily: '"Inter", sans-serif', color: 'var(--text-primary)', backgroundColor: 'transparent' };
  const genericCard = { background: 'var(--bg-card, rgba(19, 19, 26, 0.55))', border: '1px solid var(--border-color, rgba(255, 255, 255, 0.06))', borderRadius: '12px', padding: '30px', backdropFilter: 'blur(12px)', transition: 'background 0.3s, border-color 0.3s' };
  const mainHeader = { fontSize: '40px', fontWeight: '800', color: 'var(--text-primary)', margin: '0 0 15px 0', letterSpacing: '-0.02em' };
  const inputStyle = { width: '100%', padding: '14px', backgroundColor: 'var(--bg-input, #08080A)', border: '1px solid var(--border-color, rgba(255,255,255,0.1))', borderRadius: '8px', fontSize: '14px', color: 'var(--text-primary)', marginBottom: '20px', boxSizing: 'border-box', outline: 'none', transition: 'background 0.3s, border-color 0.3s' };
  const labelStyle = { display: 'block', fontSize: '11px', fontWeight: '700', color: 'var(--text-secondary)', letterSpacing: '0.06em', marginBottom: '8px', textTransform: 'uppercase' };
  
  // Department filter tags layout matrix
  const filterBadge = (isActive) => ({ padding: '8px 16px', borderRadius: '20px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', border: isActive ? '1px solid #5B3FFF' : '1px solid var(--border-color, rgba(255,255,255,0.08))', backgroundColor: isActive ? '#5B3FFF' : 'var(--bg-card, rgba(255,255,255,0.02))', color: 'var(--text-primary)', transition: 'all 0.2s', marginTop: '5px' });
  ...
  return (
    <div style={pageContainer}>
      ...
      <section style={{ marginBottom: '80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '40px', alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <h3 style={{ fontSize: '20px', color: 'var(--text-primary)', margin: 0, fontWeight: '700', letterSpacing: '-0.01em' }}>
              Corporate Parameter & Scope
            </h3>
            <p style={{ fontSize: '15.5px', color: 'var(--text-primary)', lineHeight: '1.7', margin: 0, fontWeight: '400' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ ...genericCard, padding: '22px', borderLeft: '4px solid #5B3FFF', background: 'var(--bg-card)' }}>
              <h4 style={{ color: 'var(--text-primary)', fontSize: '13px', margin: '0 0 8px 0', fontWeight: '700', letterSpacing: '0.05em' }}>WHO-GMP COMPLIANCE</h4>
            ...
            <div style={{ ...genericCard, padding: '22px', borderLeft: '4px solid #00E5FF', background: 'var(--bg-card)' }}>
              <h4 style={{ color: 'var(--text-primary)', fontSize: '13px', margin: '0 0 8px 0', fontWeight: '700', letterSpacing: '0.05em' }}>STRATEGIC GROWTH MATRIX</h4>
              <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: 0, lineHeight: '1.5' }}>
      ...
      {/* 👥 SECTION 2: DYNAMIC LIVE TEAM ROSTER GRID */}
      <section style={{ marginBottom: '80px', borderTop: '1px solid var(--border-color, rgba(255,255,255,0.08))', paddingTop: '50px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '20px', marginBottom: '35px', borderBottom: '1px solid var(--border-color, rgba(255,255,255,0.08))', paddingBottom: '20px' }}>
          <div>
            <h2 style={{ fontSize: '22px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 5px 0' }}>Professional Roster Node</h2>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>Live directory synchronized seamlessly with the administrative storage core.</p>
          </div>
        ...
        {loadingTeam ? (
          <p style={{ color: 'var(--text-secondary)', fontStyle: 'italic', textAlign: 'center' }}>Querying live system cluster registries...</p>
        ) : filteredTeam.length === 0 ? (
          <div style={{ padding: '40px', textTransform: 'uppercase', letterSpacing: '0.04em', fontSize: '13px', textAlign: 'center', color: 'var(--text-secondary)', border: '1px dashed var(--border-color, rgba(255,255,255,0.08))', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))', gap: '25px' }}>
            {filteredTeam.map((member) => {
              // 🛠️ Self-Healing Robust Path Resolution Matrix for Production & Local Environments
              let memberImg = '';
              if (member.image_url) {
                if (member.image_url.startsWith('http')) {
                  memberImg = member.image_url;
                } else {
                  const cleanPath = member.image_url.startsWith('/') ? member.image_url : `/${member.image_url}`;
                  memberImg = `${BASE_URL}${cleanPath}`;
                }
              }
              ...
              return (
                <div key={member.id} style={{ ...genericCard, padding: '24px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ position: 'relative', marginBottom: '16px' }}>
                    {memberImg ? (
                      <img 
                        src={memberImg}
                        alt={member.name}
                        style={{ width: '90px', height: '90px', borderRadius: '50%', objectFit: 'cover', border: '2px solid #5B3FFF', padding: '3px', background: 'var(--bg-input, #08080A)' }}
                        onError={(e) => {
                          // 🔄 Self-healing automatic fallback handler for production assets
                          ...
                          if (!e.target.dataset.retried) {
                            ...
                            if (currentSrc.includes('/public/')) {
                              ...
                              e.target.src = currentSrc.replace(`${BASE_URL}/uploads/`, `${BASE_URL}/public/uploads/`);
                            }
                          } else {
                    ...
                    <div style={{ display: memberImg ? 'none' : 'flex', width: '90px', height: '90px', borderRadius: '50%', backgroundColor: 'var(--bg-input, #08080A)', border: '1px solid var(--border-color, rgba(255,255,255,0.1))', alignItems: 'center', justifyContent: 'center', fontSize: '32px' }}>
                      👤
                    </div>
                  ...
                  <h4 style={{ margin: '0 0 4px 0', fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)' }}>{member.name}</h4>
                  <p style={{ margin: '0 0 12px 0', fontSize: '12px', color: '#5B3FFF', fontWeight: '600', letterSpacing: '0.02em' }}>{member.role?.toUpperCase()}</p>
                  <span style={{ fontSize: '10px', backgroundColor: 'var(--bg-input, rgba(255,255,255,0.04))', border: '1px solid var(--border-color, rgba(255,255,255,0.06))', color: 'var(--text-secondary)', padding: '2px 8px', borderRadius: '4px', display: 'inline-block', marginBottom: '14px', fontWeight: '700' }}>
                    {member.department || 'General Operations'}
                  </span>
                  
                  <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-secondary)', lineHeight: '1.5', borderTop: '1px solid var(--border-color, rgba(255,255,255,0.04))', paddingTop: '12px' }}>
      ...
      <section style={{ marginBottom: '80px', ...genericCard, padding: '40px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '0.8fr 1.2fr', gap: '40px', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: '28px', fontWeight: '700', color: 'var(--text-primary)', margin: '8px 0 0 0' }}>Our Institutional Vision</h2>
          </div>
          <div>
            <p style={{ fontSize: '15px', color: 'var(--text-primary)', lineHeight: '1.7', margin: '0 0 15px 0' }}>
      ...
      <section style={{ marginBottom: '80px' }}>
        <div style={{ marginBottom: '30px' }}>
          <h2 style={{ fontSize: '22px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 5px 0' }}>Our Core Values</h2>
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>Four fundamental pillars guiding every transactional decision and operational action.</p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
          {coreValues.map((val, idx) => (
            <div key={idx} style={{ ...genericCard, padding: '25px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ fontSize: '24px' }}>{val.icon}</div>
              <h4 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', margin: 0 }}>{val.title}</h4>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: '1.6', margin: 0 }}>{val.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ marginBottom: '80px', borderTop: '1px solid var(--border-color, rgba(255,255,255,0.08))', paddingTop: '50px' }}>
        <div style={{ marginBottom: '40px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 5px 0' }}>Our Corporate Journey</h2>
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>The lineage, progression, and expansion metrics of Eryx Biotech.</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ ...genericCard, display: 'grid', gridTemplateColumns: '120px 1fr', gap: '20px', alignItems: 'start' }}>
            ...
            <div>
              <h4 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 8px 0' }}>The Mission Conception</h4>
          ...
          <div style={{ ...genericCard, display: 'grid', gridTemplateColumns: '120px 1fr', gap: '20px', alignItems: 'start' }}>
            ...
            <div>
              <h4 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 8px 0' }}>Formal Establishment</h4>
              <p style={{ fontSize: '13.5px', color: 'var(--text-secondary)', lineHeight: '1.6', margin: 0 }}>
          ...
          <div style={{ ...genericCard, display: 'grid', gridTemplateColumns: '120px 1fr', gap: '20px', alignItems: 'start' }}>
            <div style={{ fontSize: '14px', fontWeight: '800', color: 'var(--text-primary)', background: 'var(--bg-input, rgba(255,255,255,0.06))', padding: '6px 12px', borderRadius: '6px', textAlign: 'center', border: '1px solid var(--border-color, rgba(255,255,255,0.1))' }}>
              PRESENT
            </div>
            <div>
              <h4 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 8px 0' }}>Active Operations Scaling</h4>
              <p style={{ fontSize: '13.5px', color: 'var(--text-secondary)', lineHeight: '1.6', margin: 0 }}>
      ...
      <section style={{ marginBottom: '80px', borderTop: '1px solid var(--border-color, rgba(255,255,255,0.08))', paddingTop: '50px' }}>
        <div style={{ marginBottom: '40px' }}>
          <h2 style={{ fontSize: '22px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 5px 0' }}>Regulatory & Compliance Artifacts</h2>
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>Verified documentation supporting institutional framework operations.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '30px' }}>
          ...
          <div style={{ ...genericCard, display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
            <div style={{ width: '100%', height: '480px', backgroundColor: 'var(--bg-input, #0A0A0F)', borderRadius: '8px', border: '1px solid var(--border-color, rgba(255, 255, 255, 0.08))', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', boxShadow: 'inset 0 0 20px rgba(0,0,0,0.6)', transition: 'background 0.3s' }}>
              <img 
                src={msmeImg} 
              ...
              />
              <div style={{ display: 'none', flexDirection: 'column', alignItems: 'center', color: 'var(--text-secondary)', fontSize: '13px' }}>📄 MSME CERTIFICATE SLOT</div>
            </div>
            <div style={{ borderTop: '1px solid var(--border-color, rgba(255,255,255,0.04))', paddingTop: '12px' }}>
              <h4 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 6px 0' }}>MSME Certification</h4>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0, lineHeight: '1.4' }}>Registered organizational status validation artifact issued by the ministry.</p>
            </div>
          </div>
          ...
          <div style={{ ...genericCard, display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
            <div style={{ width: '100%', height: '480px', backgroundColor: 'var(--bg-input, #0A0A0F)', borderRadius: '8px', border: '1px solid var(--border-color, rgba(255, 255, 255, 0.08))', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', boxShadow: 'inset 0 0 20px rgba(0,0,0,0.6)', transition: 'background 0.3s' }}>
              <img 
                ...
                style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '10px' }}
              />
              <div style={{ display: 'none', flexDirection: 'column', alignItems: 'center', color: 'var(--text-secondary)', fontSize: '13px' }}>📄 TRADEMARK REGISTRATION SLOT</div>
            </div>
            <div style={{ borderTop: '1px solid var(--border-color, rgba(255,255,255,0.04))', paddingTop: '12px' }}>
              <h4 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 6px 0' }}>Trademark Registration</h4>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0, lineHeight: '1.4' }}>Official certificate of protection registered under corporate trademark codes for Eryxovit.</p>
            </div>
          </div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '50px', alignItems: 'start', borderTop: '1px solid var(--border-color, rgba(255,255,255,0.08))', paddingTop: '50px' }}>
        <div>
          <p style={{ fontSize: '11px', fontWeight: '700', color: '#5B3FFF', letterSpacing: '0.06em', margin: '0 0 8px 0' }}>JOIN THE CLINICAL ARCHITECTURE</p>
          <h2 style={{ fontSize: '28px', fontWeight: '700', color: 'var(--text-primary)', margin: '0 0 15px 0' }}>Open Application Gateway</h2>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-secondary)', background: 'var(--bg-input, rgba(255,255,255,0.01))', border: '1px solid var(--border-color, rgba(255,255,255,0.04))', padding: '15px', borderRadius: '6px' }}>
        ...
        <div style={genericCard}>
          <form onSubmit={handleApplySubmit}>
            {formMessage.text && (
              <div style={{ padding: '12px 16px', borderRadius: '6px', fontSize: '13px', marginBottom: '20px', backgroundColor: formMessage.type === 'success' ? 'rgba(16, 185, 129, 0.12)' : 'rgba(239, 68, 68, 0.12)', border: formMessage.type === 'success' ? '1px solid #10B981' : '1px solid #EF4444', color: 'var(--text-primary)' }}>
                {formMessage.text}
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: '20px' }}>
              <div style={{ flex: 1 }}>
                ...
                <select value={position} onChange={(e) => setPosition(e.target.value)} style={inputStyle}>
                  <option value="Research Associate" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Research Associate</option>
                  <option value="Biotech Engineer" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Biotech Engineer</option>
                  <option value="Infrastructure QA" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Infrastructure QA</option>
                  <option value="Compliance Specialist" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Compliance Specialist</option>
                </select>
              </div>
              <div style={{ flex: 1 }}>
                ...
                <select value={experience} onChange={(e) => setExperience(e.target.value)} style={inputStyle}>
                  <option value="Entry Level (0-2 Yrs)" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Entry Level (0-2 Yrs)</option>
                  <option value="Mid-Level (3-5 Yrs)" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Mid-Level (3-5 Yrs)</option>
                  <option value="Senior Executive (5+ Yrs)" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Senior Executive (5+ Yrs)</option>
                </select>
              </div>
            </div>
